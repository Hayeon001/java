		...
		while(true) {
			...
			if(cmd.equals("q")) {
			 ...
			 //register
				...
				String name = sc.next();
				Cust cust = new Cust(id, pwd, name, 20);  //나이는 임의로 20 default
			 ...
			 //login
				...
				try {
					shopService.login(id, pwd); //login 마우스 올리면 //예외 발생할수있고 cust사용한다
					...
					while(true) {
						...
						if(cmn.equals("e")) {
							...
							Cust custInfo = null;
							custInfo = custService.get(cust.getId()); //로그인 성공시 수집한 id정보 사용
